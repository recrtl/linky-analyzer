...
var tempoColorByDay = new Map<string, tempoColor>();
...
function parseSeason(input: string) {
    interface tempoInput {
        values: Object
    }

    const data: tempoInput = JSON.parse(input)
    for (const property in data.values) {
        if (property.endsWith('-fallback')) continue
...
export class tempo {
    ...
    RunModel(input: ModelInput): ModelResult {
        ...
        const total = totalBlueHC + totalBlueHP + totalWhiteHC + totalWhiteHP + totalRedHC + totalRedHP + subscription

        const kwhTotal = this.kwhBlueHC + this.kwhBlueHP + this.kwhWhiteHC + this.kwhWhiteHP + this.kwhRedHC + this.kwhRedHP
        return {
            Name: 'EDF tempo',
            Cost: total,
            Description: `
...
const tempoData2023_2024: string = `
{
    "values": {
    "2023-11-11": "BLUE",
    "2023-11-10": "BLUE",
